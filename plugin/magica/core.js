...
(function () {
	...
	mg.FormValidator.prototype._analayzeValidators = function(input, expression)
	{
		...
		input.addEventListener('blur', function(e) {
			if(_mgFocused == null) {
				if(formValidator.validateInput(this) == false) {
					/*this.value = "";*/
					/*mg.FormValidator.validateInput(this);*/
					_mgFocused = this;
					setTimeout(function() {
						_mgFocused.focus();
						_mgFocused = null;
	...
	mg.FormValidator.prototype._validate = function()
	{
		var messages = [];
		for(var i = 0; i < this.form.elements.length; i++) {
	...
	mg.FormValidator.prototype._validateInput = function(input, requiredMessage)
	{
	...
	_MG_INTEGER_CHAR_SET = createKeySet("+-0123456789");
	_MG_DECIMAL_CHAR_SET = createKeySet("+-0123456789.");
	...
	var getJisCharSet = function () 
	{
		if(_mgJisCharSet == null) {
			_mgJisCharSet = createKeySet(JIS_CHARS);
		}
